  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        // Fetch orders: all for staff, only own for others
        let myOrders;
        if (profileData?.role === 'canteen staff') {
          // Staff: fetch all orders
          const { data } = await supabase
            .from('orders')
            .select('id,status,total_price,created_at,token_number,user_id')
